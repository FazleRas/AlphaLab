...
- Interactive price chart with SMA overlays, line/candlestick toggle, volume, RSI and MACD panes under one crosshair, timeframe selector, percent change on hover, and a **two-point measurement tool** (click any two points to measure the % / $ / day move between them)
...
- Index tape under the nav (SPY, QQQ, DIA, IWM, VIX) with an NYSE session marker, refreshed every minute
...
- `GET /quotes?tickers=SPY,QQQ,^VIX` — compact price + day change for several symbols (powers the index tape; fast_info only, so it stays clear of Yahoo's rate-limited quote endpoint)
- `GET /quote/{ticker}` — price, previous close, day change (measured against the previous session's close), high/low, 52-week high/low, volume, market cap, PE ratio
